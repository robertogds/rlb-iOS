root.getDayOfWeekString = (thisDate)->
  ...
  switch thisDay
    when 0 then return L('monday')
    when 1 then return L('tuesday')
    when 2 then return L('wednesday')
    when 3 then return L('thursday')
    when 4 then return L('friday')
    when 5 then return L('saturday')
    when 6 then return L('sunday')

root.getShortMonthString = (thisDate)->
  thisMonth = thisDate.getMonth()
  switch thisMonth
    when 0 then return L('jan').substr(3)
    when 1 then return L('feb').substr(3)
    when 2 then return L('mar').substr(3)
    when 3 then return L('apr').substr(3)
    when 4 then return L('may').substr(3)
    when 5 then return L('jun').substr(3)
    when 6 then return L('jul').substr(3)
    when 7 then return L('aug').substr(3)
    when 8 then return L('sep').substr(3)
    when 9 then return L('oct').substr(3)
    when 10 then return L('nov').substr(3)
    when 11 then return L('dec').substr(3)
